...
class SimulateData:
    ...
    def __init__(self, param, loci_coordinates, psf_files):
        ...
        self.gain = numpy.random.normal(
            loc=2.0, scale=0.1, size=(self.Dx, self.Dy)
        )
        self.offset = numpy.random.randint(
            95, high=106, size=(self.Dx, self.Dy)
        ).astype(numpy.float)
        ...
        for n_probe in range(coordinates.shape[0]):
            ...
            if x!=0 and y!=0 and z!=0:
                ...
                for dz in range (int(dzmin), int(dzmax), 1):
                    ...
                    if z>=0 and z_psf>=0 and z<=self.Lz-1 and z_psf<=self.Lz_psf-1:
                        
                        im = self.stack[:,:,int(z)]
                        im_psf = psf[int(50+dz)] * np.random.poisson(self.I)
                        
                        im[int(x-self.Lx_psf/2):int(x+self.Lx_psf/2), 
                           int(y-self.Ly_psf/2):int(y+self.Ly_psf/2)] = im_psf + im[int(x-self.Lx_psf/2):int(x+self.Lx_psf/2), 
                           int(y-self.Ly_psf/2):int(y+self.Ly_psf/2)]  
    ...
    def create_loci_stack(self):
        ...
        with tifffile.TiffWriter(name) as tf:
            ...
            for n in range(self.Lz):    
                ...
                tf.save(numpy.round(im).astype(numpy.uint16))
